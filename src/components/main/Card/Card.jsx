...
const useDeleteTesis = (tesisId, onDeletedSuccess, setModalState) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.error("Error al eliminar la tesis:", err.response?.data || err.message);
            // Mostrar modal de ERROR
            setModalState({ 
                isOpen: true, 
                status: "error", 
                message: err.response?.data?.message || "Error al eliminar la tesis. Intente de nuevo." 
            });
        } finally {
            setIsDeleting(false);
...
const Card = ({ data, isLoading = false, onTesisDeleted }) => { 
    ...
    // 💡 SOLUCIÓN DEL ERROR: Aseguramos que data exista si no estamos en modo carga.
    // Si no hay data y no estamos cargando, no renderizamos el contenido.
    if (!data && !isLoading) {
        return null; 
    }
    ...
    const handleConfirmDelete = (e) => {
        ...
        if (window.confirm(`¿Estás seguro de que quieres eliminar la tesis "${data.nombre}" (ID: ${data.id})? Esta acción es irreversible.`)) {
